...
def extract_hist(qcursor):
    ...
    for k, v in Weather.SQL_MAPPING.items():
        ...
        for trip in trips:
            ...
            for key, val in trip.items():
                if key.startswith(weather_prefix):
                    Weather.append_hist(hist_data['trip_weather'], key[len(weather_prefix):], val)
